...
function [start_points, end_points] = precise_random_walk_hyperbolic(lambda, eps, ...
    ...
    z = MCF(1i);  % Start at (0, 0.1) to avoid the real axis (decimal precision gets bad down there)
    
    % Pre-allocate array for storing points of quasi-geodesic
    max_segments = ceil(lambda/step_size * (target_length + eps)) + 1;
    start_points(max_segments, 1) = MCF;
    end_points(max_segments, 1) = MCF;
                    ...
                    for t_i = ti_inside_eps_ball
                        ...
                        for i = 0:(segment_splits - 1)
                            ...
                            if lower_bound >= 0
                                ...
                                range_i = precise_get_range_tn_routine(z, e1, e2, jump_size, ...
        ...
        for t_i = 1:(t_n - 1)
            ...
            for i = 0:(segment_splits - 1)
                ...
                %if sub_segment.dist_from_point(z) + step_size <= s
                    ...
                    %continue
                %end
                if dist_H(z, sub_segment_start) <= step_size + s + step_size/2 ...
                   || dist_H(z, sub_segment_end) <= step_size + s + step_size/2 ...
                    range_i = precise_get_range_tn_routine(z, e1, e2, step_size, ...
                                                                lowerBd1, s, ...
                                                                a, b, c, d, ...
                                                                sub_segment, true);
                    if range_i == [0, 0]
                        error("This is an extremely unfortunate circumstance you have found your poor self in." + ...
                              "If you are triggering this error message, cry.")
                    end
                    allowed_ranges = [allowed_ranges; range_i];
                end
            end
